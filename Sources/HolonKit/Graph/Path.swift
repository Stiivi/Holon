...
public class Path: Equatable {
    // TODO: Change edges from [Edge] to [ObjectID]
    ...
    public var origin: ObjectID { edges.first!.origin }

    /// Target of the path – target of the very last item in the path.
    public var target: ObjectID { edges.last!.target }
    
    public static func ==(lhs: Path, rhs: Path) -> Bool {
        guard lhs.edges.count == rhs.edges.count else {
            return false
        }
    ...
    ///
    public func joins(_ node: ObjectID) -> Bool {
