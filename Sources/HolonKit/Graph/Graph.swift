...
public class Graph {
    ...
    public init(nodes: [Node] = [], edges: [Edge] = []) {
        ...
        for node in nodes {
            assert(_nodes[node.id] == nil, "Duplicate node \(node.id)")
            _nodes[node.id] = node
        }
        for edge in edges {
            assert(_edges[edge.id] == nil, "Duplicate edge \(edge.id)")
            assert(_nodes[edge.origin] != nil, "Missing origin node \(edge.origin)")
            assert(_nodes[edge.target] != nil, "Missing target node \(edge.origin)")
            _edges[edge.id] = edge
        }
    }
    ...
    public func remove(node oid: ObjectID) -> [Edge] {
        guard let node = _nodes[oid] else {
            preconditionFailure("The graph does not contain a node with id '\(oid)'")
        }
        
        var disconnected: [Edge] = []
        
        // First we remove all the connections
        for edge in Array(_edges.values) {
            if edge.origin == oid || edge.target == oid {
    ...
    /// errors.
    ///
    @available(*, deprecated, message: "Create and populate connection object separately")
    @discardableResult
    public func connect(from origin: Node,
                        to target: Node,
                        labels: LabelSet=[],
                        id: OID?=nil) -> Edge {
        let edge = Edge(origin: origin.id,
                        target: target.id,
                        labels: labels,
                        id: id)
    ...
    public func add(_ edge: Edge) {
        precondition(_edges[edge.id] == nil,
                     "The graph already contains an edge with id '\(edge.id)'.")
        precondition(_nodes[edge.origin] != nil,
                     "The graph does not contain the origin '\(edge.origin)' of edge '\(edge.id)'")
        precondition(_nodes[edge.target] != nil,
                     "The graph does not contain the target '\(edge.origin)' of edge '\(edge.id)'")
    ...
    ///
    public func remove(edge: ObjectID) {
        precondition(_edges[edge] != nil,
                     "The graph does not contains an edge with id '\(edge)'.")
